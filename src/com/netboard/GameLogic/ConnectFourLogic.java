...
public class ConnectFourLogic implements Logic {
	...
	// Check rows, if there are 4 or more disks of the same color - return
	// winner color
	...
	// Check columns, if there are 4 or more disks of the same color - return
	// winner color
	...
	// Check diagonals, if there are 4 or more disks of the same color - return
	// winner color
	private static char getWinnerInDiagonals(char[][] field) {
		// There are 2 kinds of diagonals, let's check those that go from
		// top-left to bottom right

		// There are diagonals, that starts on top of each column, let's check
		// them
		for (int column = 0; column < 7; ++column) {
			int count = 0;
			// Traverse diagonal that starts at [0][column], we start with the
			// first row,
			// because we will compare elements with the previous one, similar
			// to how
		...
		// There are diagonals, that starts on left of each row, let's check
		// them
		for (int row = 0; row < 7; ++row) {
			int count = 0;
			// Traverse diagonal that starts at [row][0], we start with the
			// first column,
			// because we will compare elements with the previous one, similar
			// to how
			// we did this for rows and columns
		...
		// Now we need to do the same for diagonals that go from top-right to
		// bottom-left
		// There are diagonals, that starts on top of each column, let's check
		// them
		for (int column = 0; column < 7; ++column) {
	...
	public static char getWinner(char[][] field) {
		...
		// Now we need to check if there are empty positions, otherwise it is a
		// draw
	...
	public String getWinner() {
		// gets the player who made the last move once isGameOver() == true
		return "";
	}

	public boolean updateBoard(Piece[][] board, Piece p) {
		// make a move and returns false if the move is invalid
		return false;
	}
